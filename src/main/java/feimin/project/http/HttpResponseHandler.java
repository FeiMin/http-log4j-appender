...
/**
 * The response handler for the HTTP request. This class is meant to be used as an anonymous inner class when making the
 * HTTP request and will return the content as a byte array. You may also inherit this class if you want to handle the
 * content in a unique way. For example: {@link FileHttpResponseHandler}, {@link StringHttpResponseHandler}. <p />
 * Example: <pre> client.get(url, params, new HttpResponseHandler() { &#064;Override public void onSuccess(int
 * statusCode, Map<String, List<String>> headers, byte[] content) { // Request was successful }
 *
 * &#064;Override public void onFailure(int statusCode, Map<String, List<String>> headers, byte[] content) { // Server
 * responded with a status code 4xx or 5xx error }
 *
 * &#064;Override public void onFailure(Throwable throwable) { // An exception occurred during the request. Usually
 * unable to connect or there was an error reading the response } }); </pre>
 *
...
public abstract class HttpResponseHandler {
    ...
    /**
     * Called after the {@link HttpURLConnection} object is prepared, but before a connection or the request is actually
     * made. This method is meant to be overridden and is best used if any additional changes need to be made to the
     * {@link HttpURLConnection} object before making the request.
     *
    ...
    /**
     * Called after the request has finished and all data has been received. This method is the very last callback,
     * coming after any onSuccess() or onFailure() methods.
     *
    ...
    /**
     * Called when the request was successful and contains response information. This method is meant to be overridden
    ...
    /**
     * Called when the request failed and the server issued an error code. This method is meant to be overridden in an
    ...
    /**
     * Called every time the buffer fills while reading the response content. Best used when downloading large files or
    ...
    /**
     * Reads from the response body {@link InputStream}. This method can be overridden if you need to read the content
     * in a different way.
     *
    ...
    protected byte[] readFrom(InputStream inputStream, long length) throws IOException {
        ...
        byte[] buffer = new byte[HttpResponseHandler.BUFFER_SIZE];
        int bytesRead;
